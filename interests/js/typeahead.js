	...
	fetch: function() {
		...
		$.ajax({
			...
			try {
				...
				_.each( json, function(item, ix, list){
					userNames.push( item['INTEREST'] );
					userObjs[ item['INTEREST'] ] = item;
					process( userNames );	
				});
			} catch (ev) {
				console.log(json);
		...
		$(".typeahead").typeahead({
			...
			highlighter: function( item ){
          		...
          		var suggestElement = '<div class="user" style="padding: 5px;">';
          		suggestElement += '<strong>'+user['INTEREST']+'</strong><br /></div>';
